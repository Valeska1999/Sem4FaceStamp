...
class face_recognition:
    def __init__(self, root):
        ...
        # Background Image
        img_path = r"C:\Users\Valeska\Desktop\Sem4MiniProject\ImagesForGUI\page2.jpeg"
    ...
    def mark_attendace(self,i,r,n):
        with open("att.csv","r+",newline="\n") as f:
            ...
            if not rows:
                ...
                for row in rows:
                    if i not in row and r not in row and n not in row:
                        # If the student is not present, insert attendance record into the database
                        now = datetime.now()
                        d1 = now.strftime("%d/%m/%Y")
                        dtString = now.strftime("%H:%M:%S")  # Specify the format string
                        
                        # Insert the attendance record into the database
                        insert_query = "INSERT INTO attendance (spid, rollno, sname, date, time, status) VALUES (%s, %s, %s, %s, %s, %s)"
                        attendance_data = (i, r, n, d1, dtString, "Preset")
                        my_cursor.execute(insert_query, attendance_data)
                        conn.commit()
                    else:
                        print("Attendance already marked for student.")
    ...
    def face_recog(self):
        ...
        def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            for (x, y, w, h) in features:
                ...
                # d = "+".join(d)
                if confidence > 77:
                    cv2.putText(img, f"spid: {i}", (x, y-75), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
...
#                 conn = mysql.connector.connect(host="localhost", username="root", password="$Leaval10", database="sem4facestamp")
#                 my_cursor = conn.cursor()
#                 query1 = "INSERT INTO student VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
#                 data1 = (self.var_dep.get(), self.var_course.get(), self.var_year.get(), self.var_sem.get(), self.var_pid.get(), self.var_name.get(), self.var_div.get(), self.var_rollno.get())
#                 my_cursor.execute(query1, data1)
#                 query2 = "INSERT INTO attendance VALUES (%s, %s, %s)"
#                 data2 = ( self.var_pid.get(), self.var_name.get(), self.var_rollno.get())
